...
直至找到了相同的一组节点，他就会移将对应的指针向没有处理过的节点方向移动，再次进行上述方式的比较。比如这里就是newStartId指针向下移动，而oldEndIdx指针向上移动。如下图：
...
新旧节点各还有一个节点没有比完，有的人会产生疑问，为啥要对比p-2和p-2不是一样的吗，不过对于程序来说，还没有移动指针，所以它是不清楚是否是相同节点的，所以还需要继续对比，将newEndIdx向上移动，oldStartIdx向下移动。

![alt text](image-32.png)

1. newEndIdx指针向上移动和newStartIdx重合，所以新子节点的头部节点和尾部相同，都是p-2。
2. oldStartIdx指针向下移动和oldEndIdx重合，所以旧子节点的头部节点和尾部节点相同，都是p-2。

此时对比还没有结束，依次重复上述步骤：

1. 第一步：比较旧的一组子节点中的头部节点p-2与新的一组子节点中的头部节点p-2是否相同。由于两者的key相同，因此可以进行DOM复用。
